...
from backend.seed_content import seed_content
...
# DnD content libraries (spells, items, feats, backgrounds, lore) are no longer
# hardcoded here. They live as git-tracked JSON in backend/seed_data/ (captured
# by export_content.py) and load via backend.seed_content — the single source of
# truth. See seed_content.seed_content().
...
async def main():
    ...
    # DnD content libraries load from the git-tracked JSON snapshots in
    # backend/seed_data/ (spells, items, feats, backgrounds, encyclopedia lore).
    # INSERT OR IGNORE tops up only what's missing, so this never disturbs your
    # in-app edits. Regenerate the snapshots with `python -m backend.export_content`.
    print("\nSeeding/topping up DnD content libraries from seed_data/ ...")
    await seed_content()
